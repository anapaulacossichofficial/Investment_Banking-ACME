...
A hybrid enterprise architecture combining a **Salesforce Agentforce target deployment** and a local engineering runtime. This repository serves as the definitive artifact for the **Partner Technical Architect (Agentforce)** Option B panel evaluation.

---

## 🚨 Option B Technical Panel: Start Here
...
> To simulate a realistic enterprise client experience, this solution goes beyond the baseline Meeting Prep requirements. We built an **Engineering Lab Runtime (Streamlit)** that not only fulfills the Option B knowledge-grounding criteria but also introduces an advanced **Competitive Intelligence & Market Benchmarking** workflow comparing industry peers. This exact end-to-end scenario was successfully transitioned from the local lab and fully validated natively inside the **ACME Agentforce Dev Org Working Build** (complete with test results and proofs). This proves a seamless, end-to-end leap from local simulation to a production-ready enterprise cloud deployment.
...
## ☁️ ACME Dev Org runtime - ACME Dev Working-build with Agentforce

This repository is tightly coupled with a Salesforce Agentforce Developer Edition Org to validate native AI orchestration. The source code is located in `acme-agentforce/force-app/main/default/`.
...
| Known Integrations | Apex Invocables (`ACME_CompetitiveIntelligenceAction`), Data Cloud Grounding (`ACME_Meeting_Prep_Knowledge`), Named Credentials (MCP/A2A patterns) |
...
## 💻 Engineering Runtime & Visual Preview (Lab Runtime)

<details>
<summary><b>Click to expand: Executive Visual Preview & Simulated Environment Evidence</b></summary>
<br>

The repository includes a local engineering working build (Streamlit) designed to simulate the real-world end-to-end client experience. It acts as a sandbox for the external intelligence backend, validating deterministic engineering contracts and competitive benchmarking workflows before cloud deployment.
...
### Validation Evidence (Agentforce & Local)
...
Repository Tag: v0.9.8 | Status: Option B Fully Validated.
