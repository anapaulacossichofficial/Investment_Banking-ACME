# ACME Investment Banking Working Build & Agentforce Blueprint
A hybrid enterprise architecture combining an Enginnering working build for executive meeting preparation and a **Salesforce Agentforce target deployment** validated against the Option B architectural deliverables.

## 🏆 Option B Deliverables (Final Audit)

This repository serves as the definitive artifact for the Agentforce Option B Technical Panel. All 5 deliverables have been fully validated and committed to the `main` branch (Tag `v0.9.6`):

1. **Architecture Diagram:** Bounded flows between Agent, Actions, CRM, and External API documented in `docs/architecture/`.
2. **Working Build (E2E):** Validated conversational routing, CRM grounding (`Account Snapshot`), and Apex Code Coverage (**>75%** in `docs/evidence/salesforce-org/2026-08-26_apex-test-coverage.json`). Full UI tracing available in `docs/evidence/salesforce-org/`.
...
4. **Platform Judgment Callout:** Navigated the architectural trade-off of disabling `Session Activation Required` for autonomous M2M Agent callouts to prevent silent Trust Layer authorization failures.
...
| Diagram | Purpose |
|---|---|
| [Solution HLD](docs/architecture/3_ACME_Investment_Bank_HLD_Architecture.png) | High-level design of the proposed solution: banker interaction, Salesforce UX, Agentforce Supervisor/Planner, Meeting Prep Agent, CRM/Data Cloud retrieval, Einstein Trust Layer, and enterprise interoperability with 30+ agents |
...
| [Design Trade-offs](docs/architecture/2_acme-agentforce-pattern_Tradeoffs.png) | Consolidated trade-offs view referencing [docs/architecture/design-trade-offs.md](docs/architecture/design-trade-offs.md) |
...
> **Scope:** These screenshots and traces document the target-state Agentforce sandbox configuration and validation evidence. They complement the local Streamlit working build and demonstrate alignment between deterministic engineering contracts and the Salesforce platform target. **Final E2E Traces for Option B are located in `docs/evidence/salesforce-org/`.**
...
| Known Open Issue | Fixed 2026-08-26 — misplaced retriever method in `pages/2_Competitive_Intelligence.py` |
